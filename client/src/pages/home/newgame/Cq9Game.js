...
const Cq9Game = () => {
  ...
  const handleJilliOpen = (data) => {
    ...
    debounce(() => {
      dispatch(gameListByProvider({ provider: "cq9", page:1, size:5 })).then(
        (res) => {
     ...
     <div className="mb-8">
                     ...
                     <div className="games-grid mt-1 grid grid-cols-3 gap-2 px-1">
                       {gameList&& gameList?.map((game, index) => (
                         <div
                           key={index}
                           className="border rounded-lg shadow-sm h-[140px] hover:shadow-md bg-white"
                         >
                           <img

...
const JilliGame = () => {
  ...
  const [gameList,setGameList]=useState([]);
    ...
    const [gameType, setGameType] = useState("Crash Game");
  ...
  const handleJilliOpen = (data) => {
    ...
    debounce(() => {
      dispatch(gameListByGameTypeAndProvider({ provider: "jili",game_type:gameType, page:1, size:5 })).then(
        (res) => {

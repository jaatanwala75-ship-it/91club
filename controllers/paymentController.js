    ...
    try {
        ...
        if (!money || !(money >= 100)) {
            return res.status(400).json({
                message: `Money is Required and it should bess ₹${minimumMoneyAllowed} or above!`,
                status: false,
                timeStamp: timeNow,
            })
        }
...
const addManualUSDTPaymentRequest = async (req, res) => {
    try {
        ...
        if (!money || !(money >= minimumMoneyAllowed)) {
            return res.status(400).json({
                message: `Money is Required and it should bee ₹${minimumMoneyAllowed}  or above!`,
                status: false,
                timeStamp: timeNow,
            })
        }
...
//  return res.status(400).json({
//                 message: `statthkjhk ${PaymentMethodsMap.USDT_MANUAL}`,
//                 status: false,
//                 timeStamp: timeNow,
//             })
